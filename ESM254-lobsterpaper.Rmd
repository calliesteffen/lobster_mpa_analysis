...
## intersect fishing blocks with MPA boundaries - buffer 0 to make it work 
blocks_sf_buffer <- st_buffer(blocks_sf, 0)

#check
x <- blocks_sf_buffer %>% 
  mutate(a  = st_area(geometry))

#DO WE WANT A BUFFER? Uncomment if so

#reserve_blocks <- st_intersection(blocks_sf_buffer, st_buffer(mpa_sf, 10))

reserve_blocks <- st_intersection(blocks_sf_buffer,mpa_sf)

## create dataframe of fishing block #s within reserves (get rid of sticky geometries) + 10m buffer
...
#Generate list of control blocks that do not receive any spillover from reserves by antijoining the list of all fishing blocks with the list of reserves

control_blocks_list <- anti_join(blocks_df, reserve_blocks_df)

control_blocks_df <- as.data.frame(control_blocks_list) %>% 
  dplyr::select(block) %>% 
  distinct()
...
#add classification to each dataframe and then rbind

class_reserve <- reserve_blocks_df %>% 
  mutate(classification = "reserve")

class_control <- control_blocks_df %>% 
  mutate(classification = "control")

block_classification <- rbind(class_reserve,class_control)
...
#Joining lobster data frames and classifying as control or reserve blocks 
...
test <- full_join(lobsters,block_classification, by = c("block")) %>% 
  distinct()
...
# Visualize
lobster_study_area_map <- mapview(lobster_blocks_pb,
          zcol = "block",
          col.regions = "gray",
...
#test
          lobster_summary_lbs2 <- omit_na_lbs2 %>% 
  group_by(classification, year) %>% 
  summarise(total_traps = sum(total_traps_pulled), 
            season_pounds_landed = sum(total_pounds_landed), 
            sd_traps = round(sd(total_traps_pulled), digits = 3),             sd_pounds = round(sd(total_pounds_landed), digits = 3),
...
coastal_pounds_ba2 <- lobster_summary_lbs2 %>% 
  filter(year > 2005) %>% 
  mutate(years_ba_res = year-2011) %>% 
  mutate(log_traps = log(total_traps +1)) %>% 
  mutate(log_pounds = log(season_pounds_landed + 1)) %>% 
